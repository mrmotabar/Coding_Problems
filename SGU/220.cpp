...
int n, k;
vector<int> w, b;
Bignum dpw[10 + 10][100 + 10], dpb[10 + 10][100 + 10], ans;
int main()
{
	...
	for (int i = 1; i < w.size(); i++)
	{
		...
		for (int j = 1; j <= k; j++)
		{
			dpw[i][j] = dpw[(i - 1)][j] + (dpw[(i - 1)][j - 1] * (w[i] - (j - 1)));
		}
	}
	for (int i = 1; i < b.size(); i++)
	{
		...
		for (int j = 1; j <= k; j++)
		{
			dpb[i][j] = dpb[(i - 1)][j] + (dpb[(i - 1)][j - 1] * (b[i] - (j - 1)));
		}
	}
	for (int i = 0; i <= k; i++)
	{
		ans += dpw[w.size() - 1][i] * dpb[b.size() - 1][k - i];
	}
	ans.show();
}
